"""
reference: https://github.com/pablospe/render_depthmap_example
"""
...
class DepthRenderer:
    def __init__(
        ...
        self.__accumulated_pointcloud = deque(maxlen=maxlen)
    ...
    def add_pointcloud(self, points, voxel_size=None):
        if voxel_size is None:
            voxel_size = self.__voxel_size

        pcd = open3d.geometry.PointCloud()
        pcd.points = open3d.utility.Vector3dVector(points)
        voxels = open3d.geometry.VoxelGrid.create_from_point_cloud(
            pcd, voxel_size=voxel_size
        )

        self.add_geometry(voxels)

        while len(self.__accumulated_pointcloud) >= self.__maxlen:
            old_voxels = self.__accumulated_pointcloud.popleft()
            self.remove_geometry(old_voxels)

        self.__accumulated_pointcloud.append(voxels)
    ...
    def update_view_point_intrinsics(self, intrinsic):
        extrinsic = self.get_view_point_extrinsics()
        self.update_view_point(intrinsic, extrinsic)

    def update_view_point_extrinsics(self, extrinsic):
    ...
    def capture_screen_float_buffer(self, show=False):
        """return the RGB image (open3d.geometry.Image)"""
    ...
    def capture_depth_float_buffer(self, show=False):
        """return the depth image (open3d.geometry.Image)"""
...
if __name__ == "__main__":
    ...
    vis = DepthRenderer(visible=True, maxlen=3)

    for i in range(10):
        points = np.random.rand(100, 3)
        vis.add_pointcloud(points)
