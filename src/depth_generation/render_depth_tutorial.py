...
raster_settings = PointsRasterizationSettings(
    image_size=512, radius=0.003, points_per_pixel=10
)
...
plot_scene({"Pointcloud": {"person": point_cloud}})
...
point_cloud_batch = Pointclouds(
    points=[verts, verts + 2], features=[rgb, torch.zeros_like(rgb)]
)
# render both in the same plot in different traces
fig = plot_scene(
    {"Pointcloud": {"person": point_cloud_batch[0], "person2": point_cloud_batch[1]}}
)
fig.show()
...
fig = plot_scene({"Pointcloud": {"2 people": point_cloud_batch}})
fig.show()
...
fig3 = plot_batch_individually(
    ...
    subplot_titles=[
        "Pointcloud1",
        "Pointcloud2",
    ],  # this should have a title for each subplot, titles can be ""
