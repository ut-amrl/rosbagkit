...
def fit_ellipsoid(bboxes):
    ...
    # Initial guess
    cX, cY, cZ = np.mean(bboxes[:, :3], axis=0)
    a, b, c = np.mean(bboxes[:, 3:6], axis=0)
    ...
    # Optimization
    result = least_squares(fun, x0, loss="soft_l1")

    return result.x


def cluster_fit_3d_bboxes(bboxes, threshold=1.5, min_samples=2):
    """
...
def main(args):
    ...
    for class_name in CLASSES:
        ...
        fitted_bboxes[class_name] = cluster_fit_3d_bboxes(global_bboxes[class_name])
    ...
    if args.visualize:
        ...
        vis_class = ["Tree"]
        for class_name in vis_class:
            for fitted_result in tqdm(fitted_bboxes[class_name], desc=class_name):
                vis.clear_geometries()

                # draw fitted ellipsoids
                ell = create_o3d_ellipsoid(
                    fitted_result["ellipsoid"], CLASSES[class_name]["color"]
                )
                vis.add_geometry(ell)

                # draw cluster bboxes
                for idx in fitted_result["indices"]:
                    bbox = global_bboxes[class_name][idx]
