...
def cluster_fit_3d_bboxes(bboxes, threshold=1.0, min_samples=2, visualize=False):
    """
    ...
    for unique_label in tqdm(unique_labels):
        ...
        # Fit an 3d bbox
        fitted_bbox = fit_3d_bbox(cluster_bboxes)
        ...
        if vis:
            ...
            obb = create_o3d_3d_bbox(fitted_bbox, (0.0, 1.0, 1.0))
            vis.add_geometry(obb)
...
def main(args):
    ...
    for class_name in CLASSES:
        ...
        fitted_bboxes = cluster_fit_3d_bboxes(
...
def get_args():
    ...
    args.global_3d_bbox_dir = args.dataset_dir / "annotations" / "global_3d_bboxes"
