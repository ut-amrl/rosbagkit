...
def load_keyframe_pose(keyframe_out_posefile: str) -> np.ndarray:
    ...
    with open(keyframe_out_posefile, "r") as f:
        ...
        timestamp = float(timestamp_line[1]) + float(timestamp_line[2].zfill(9)) * 1e-9
...
def get_args():
    ...
    parser.add_argument(
        "--out_posefile",
        type=str,
        help="Path to the output file containing the estimated poses",
    )
    ...
    if args.out_posefile is None:
        ...
        args.out_posefile = Path(args.out_posefile)
        args.out_posefile.parent.mkdir(parents=True, exist_ok=True)
